...
class ModelNoticias{
    ...
    public static function mdlAddNoticias($noticia){
        try{
            $stmt = Conexion::conectar()->prepare("INSERT INTO noticias VALUES(0,:imagen,:titulo,:descripcion,:creador,:fecha)");
            $stmt->execute([
    ...
    public static function mdlUpdateNoticias($noticia){
        try{
            $stmt = Conexion::conectar()->prepare("UPDATE noticias set imagen=:image,titulo=:titulo, descripcion = :descripcion, publicador = :publicador WHERE id=:id");
            $stmt->execute([
               "image" => $noticia[0],
              "titulo" => $noticia[1]["titulo"],
              "descripcion" => $noticia[1]["descripcion"],
